...
def test_completion_command_prints_packaged_bash_script(capsys):
    assert cli.main(["completion", "bash"]) == 0
    output = capsys.readouterr().out
    assert output.startswith("# Generated by scripts/generate_cli_artifacts.py")
...
def test_completion_command_rejects_side_effecting_global_options(capsys):
    ...
    assert raised.value.code == 2
    assert "not supported by completion" in capsys.readouterr().err
...
def test_bare_human_invocation_is_a_safe_fixed_order_terminal_home(monkeypatch, capsys):
    ...
    assert captured.out.splitlines() == [
        ...
        "",
        "Shell integration (optional, offline)",
        "  jring completion bash",
        "  Print the packaged Bash completion script; it does not install or activate it.",
        "",
        ...
        "  For private protocol investigation: capabilities → observe → review-observation.",
        "  This stores bounded unknown notifications locally; it does not decode or enable behavior.",
        "",
