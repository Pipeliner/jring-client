...
def test_all_fourteen_local_ble_or_dynamic_gatt_requests_are_accounted_once():
    ...
    assert all(
        item.directly_consults_device_gear_policy is False for item in inventory
    )
    assert all(item.establishes_owner_authorization is False for item in inventory)
...
def test_connection_lifecycle_distinguishes_close_disconnect_and_retained_target():
    ...
    assert close.direct_bluetooth_effect is DirectBluetoothEffect.GATT_CLOSE
    ...
    assert connect.direct_bluetooth_effect is (
        DirectBluetoothEffect.GATT_CONNECT_WITH_RECONNECT
    )
    assert connect.argument_privacy == (
        ArgumentPrivacy.DEVICE_NAME,
        ArgumentPrivacy.BLUETOOTH_ADDRESS,
    )
    assert connect.android_local_side_effects == (
        "clears_user_disconnected",
        "resets_last_connection_state",
    )
    assert connect.effects_when_shared_sdk_validation_status_200 == (
        "stores_current_target",
        "persists_name_and_address",
        "starts_sdk_reconnect_flow",
    )
    assert connect.persisted_state is (
        PersistedState.CONDITIONALLY_STORES_NAME_AND_ADDRESS
    )
    assert connect.shared_sdk_validation_status_use is (
        SharedSdkValidationStatusUse.REQUIRES_200
    )
    assert "raw_address" in connect.unsafe_logging
    assert disconnect.direct_bluetooth_effect is DirectBluetoothEffect.GATT_DISCONNECT
...
def test_connected_device_is_a_private_local_getter_not_a_radio_query():
    ...
    assert item.direct_bluetooth_effect is DirectBluetoothEffect.LOCAL_STATE_QUERY
...
def test_connected_rssi_is_an_async_android_gatt_read_with_callback_result():
    ...
    assert item.direct_bluetooth_effect is DirectBluetoothEffect.REMOTE_RSSI_READ
    assert item.shared_sdk_validation_status_use is (
...
def test_authorization_getter_returns_shared_sdk_validation_not_owner_state():
    item = _by_operation()[LocalBleOperation.IS_AUTHORIZE]

    assert item.direct_bluetooth_effect is DirectBluetoothEffect.LOCAL_STATE_QUERY
    assert item.shared_sdk_validation_status_use is (
        SharedSdkValidationStatusUse.RETURNS_CURRENT
    )
    assert item.python_analogue is PythonAnalogue.SDK_VALIDATION_STATUS_ONLY
    assert "does_not_validate_or_contact_vendor_network" in item.static_findings
    assert "does_not_return_device_gear_policy" in item.static_findings
    assert "does_not_establish_ring_owner_authorization" in item.static_findings
...
def test_scan_toggle_is_shared_status_gated_but_python_scan_is_bounded():
    item = _by_operation()[LocalBleOperation.SCAN_DEVICE]

    assert item.direct_bluetooth_effect is DirectBluetoothEffect.ACTIVE_SCAN_TOGGLE
    ...
    assert item.android_local_side_effects == ()
    assert "android_scan_timers_and_retry_counter" in (
        item.effects_when_shared_sdk_validation_status_200
    )
...
def test_scan_mode_is_unvalidated_runtime_configuration_not_a_scan_itself():
    ...
    assert item.direct_bluetooth_effect is DirectBluetoothEffect.SCAN_CONFIGURATION
...
def test_dynamic_uuid_configuration_controls_future_subscriptions_and_write_lookup():
    ...
    assert item.direct_bluetooth_effect is (
        DirectBluetoothEffect.DYNAMIC_GATT_CONFIGURATION
    )
...
def test_dynamic_characteristic_write_is_documented_but_never_exposed():
    ...
    assert item.direct_bluetooth_effect is DirectBluetoothEffect.ARBITRARY_GATT_WRITE
    assert item.argument_privacy == (
        ArgumentPrivacy.DYNAMIC_GATT_UUIDS,
