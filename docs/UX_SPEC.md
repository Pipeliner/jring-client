...
false. The device-action, cumulative-step, Classic info/name, host-volume, and
touch-mode rows say that passive scripted fake event decoding is covered. Touch-mode
...
host-volume requests, and the exact `78/09` touch-mode setting projection. App-ID
...
unrelated. Every `78` selector other than `09` is unrelated.
...
while including fixed zero-write, no-setter, no-ring, no-gesture, no-terminal,
no-hardware, and no-input fields; private values remain available only through explicit
