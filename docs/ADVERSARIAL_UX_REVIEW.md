...
| A MAIN `54/04` callback looked like current or connected Wi-Fi state, while trailing private address bytes could leak | The exact-selector zero-write projection retains only a redacted neutral state code, discards address material, and reports unknown request relationship. Other and selectorless `54` traffic stay unrelated; fixed fields deny credentials, networking, and radio changes, say the code does not report whether Wi-Fi is enabled, connected, joined, current, or internet-reachable, and deny acknowledgement, terminal, live, hardware, and input authority |
...
| A typed offline request was changed after validation, or a colliding/truncated callback poisoned another operation and made fake output look authoritative | Opaque request families retain an identity-bound original frame seal, all eight behavior requests are reconstructed and seal-checked, and validation occurs before operation creation. Matching classifies opcode and branch discriminator before length; EQ SET-kind and selectorless shared traffic remain unrelated, while heart start/stop terminals stay disjoint. None of this grants live or hardware authority |
